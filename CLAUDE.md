...
- **Video files:** Vimeo. The CMS stores Vimeo URLs; this repository does not store source videos. Exception: each Project may optionally have a short (under 5s), low-resolution, silent "hover preview clip" uploaded directly as a Sanity file asset (same pattern as thumbnail images) — these are small enough that Sanity's own asset CDN is the right home, not Vimeo or this repo. Second exception: Site Settings may hold compressed homepage reel MP4s as Sanity file assets — a 9:16 mobile one (~720×1280, under ~10 MB) and a 16:9 desktop one (1920×1080, ~25 MB) — which each device plays directly in a native `<video>` instead of the Vimeo iframe for much faster loading. The Vimeo reel URLs remain as the fallback when a file isn't set or fails to load. Site Settings also holds two optional short, silent "loading animation" MP4s (desktop and mobile) as Sanity file assets, shown in front of the hero reel while it loads.
