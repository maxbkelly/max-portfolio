...
- **Video files:** Vimeo. The CMS stores Vimeo URLs; this repository does not store source videos. Exception: each Project may optionally have a short (under 5s), low-resolution, silent "hover preview clip" uploaded directly as a Sanity file asset (same pattern as thumbnail images) — these are small enough that Sanity's own asset CDN is the right home, not Vimeo or this repo. Second exception: Site Settings may hold a compressed 9:16 "mobile homepage reel" MP4 (~720×1280, under ~10 MB) as a Sanity file asset, which phones play directly in a native `<video>` instead of the Vimeo iframe for faster loading on mobile data. Desktop always uses the Vimeo reel. Site Settings also holds two optional short, silent "loading animation" MP4s (desktop and mobile) as Sanity file assets, shown in front of the hero reel while it loads.
