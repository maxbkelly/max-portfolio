...
- **Video files:** Vimeo. The CMS stores Vimeo URLs; this repository does not store source videos. Exception: each Project may optionally have a short (under 5s), low-resolution, silent "hover preview clip" uploaded directly as a Sanity file asset (same pattern as thumbnail images) — these are small enough that Sanity's own asset CDN is the right home, not Vimeo or this repo. Second exception: Site Settings may hold compressed homepage reel MP4s as Sanity file assets — a 9:16 mobile one (~720×1280, under ~10 MB) and a 16:9 desktop one (1920×1080, ~25 MB) — which each device plays directly in a native `<video>` instead of the Vimeo iframe for much faster loading. The Vimeo reel URLs remain as the fallback when a file isn't set or fails to load. Site Settings also holds two optional short, silent "loading animation" MP4s (desktop and mobile) as Sanity file assets, shown in front of the hero reel while it loads. Third exception: each Project may have an optional Cloudflare Stream video ID (`streamVideoId`), which phones play in a native `<video>` (HLS) in the project viewer — iPhones only allow sound and native fullscreen from a tap on a video in the page itself, not inside Vimeo's iframe. On phones, projects with a Stream video make no Vimeo requests at all: poster frames and the swipe preview come from Stream (the `streamThumbnailTime` second if set, else the video's default frame in Stream). Desktop always uses Vimeo, and phones fall back to Vimeo only when the ID is empty or the stream fails. The Stream upload token lives only in the local, git-ignored `.env.stream`.
